...
namespace EsemkaCorporation._1
{
    public partial class PromotionForm : Form
    {
        ...
        private void PromotionForm_Load(object sender, EventArgs e)
        {
            ...
            var listMutation = db.positions.Where(s => s.deleted_at == null && s.job.job_level_id == (data.job.job_level_id + 1) && s.job.head_count > (db.positions.Count(x => x.job_id == s.job_id)));
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.Columns[e.ColumnIndex].HeaderText == "Action")
            {
                ...
                var check = db.promotions.Any(s => s.job_id == id && s.employee_id == Variable.employee.id);
                if (check)
                {
